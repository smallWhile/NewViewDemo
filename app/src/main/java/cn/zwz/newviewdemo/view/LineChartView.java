...
public class LineChartView extends View {
    ...
    private int rulerValuePaddingDP = 3;//刻度单位与轴的间距默认dp
    private float heightPercent = 0.618f;

    private int lineColor = Color.parseColor("#004C97");//曲线颜色
    private float lineWidthDP = 2f;//曲线宽度dp

    private int pointColor = Color.parseColor("#004C97");//锚点颜色
    private float pointWidthDefault = 8f;
    private float pointWidthDP = pointWidthDefault;//锚点宽度dp

    private int tableColor = Color.parseColor("#f5f5f5");//表格线颜色
    private float tableWidthDP = 0.3f;//表格线宽度dp

    private int rulerTextColor = Color.parseColor("#40000000");;//表格标尺文本颜色
    private float rulerTextSizeSP = 8f;//表格标尺文本大小
    ...
    private void drawRulerXValue(Canvas canvas) {
        ...
        for (int i = 0; i < linePoints.length; i++) {
            ...
            if (point == null) break;
            drawRulerXText(canvas, "0"+ String.valueOf(i+1), linePoints[i].x, 0);
        }
    }
